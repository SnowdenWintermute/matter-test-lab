...
export default function handleWASD(game: TestGame) {
  ...
  const { angle } = body;
  if (inputState.up) Matter.Body.applyForce(body, body.position, calculateDirectionalForce(angle, DIRECTION.UP, acceleration));
  if (inputState.down)
    Matter.Body.applyForce(body, body.position, calculateDirectionalForce(angle, DIRECTION.DOWN, acceleration * reverseAccelerationModifier));
  if (inputState.left)
    Matter.Body.applyForce(body, body.position, calculateDirectionalForce(angle, DIRECTION.LEFT, acceleration * sidewaysAccelerationModifier));
  if (inputState.right)
    Matter.Body.applyForce(body, body.position, calculateDirectionalForce(angle, DIRECTION.RIGHT, acceleration * sidewaysAccelerationModifier));
}
